...
public abstract class ECPoint {
	 ...
	 *
	 * Caution: depending on the curve's coordinate system, this may not be the same value
	 * as in an affine coordinate system; use normalize() to get a point where the
	 * coordinates have their affine values, or use getAffineXCoord() if you expect the
	 * point to already have been normalized.
	 *
	 ...
	 * Caution: depending on the curve's coordinate system, this may not be the same value
	 * as in an affine coordinate system; use normalize() to get a point where the
	 * coordinates have their affine values, or use getAffineYCoord() if you expect the
	 * point to already have been normalized.
	 *
	...
	public boolean isNormalized() {
		...
		return coord == ECCurve.COORD_AFFINE || coord == ECCurve.COORD_LAMBDA_AFFINE
				|| isInfinity() || zs[0].isOne();
	}

	/**
	 * Normalization ensures that any projective coordinate is 1, and therefore that the
	 * x, y coordinates reflect those of the equivalent point in an affine coordinate
	 * system.
	 *
	 * @return a new ECPoint instance representing the same point, but with normalized
	 * coordinates
	 */
	...
	protected ECPoint createScaledPoint(ECFieldElement sx, ECFieldElement sy) {
		return this.getCurve().createRawPoint(getRawXCoord().multiply(sx),
				getRawYCoord().multiply(sy), this.withCompression);
	}
	...
	public ECPoint scaleX(ECFieldElement scale) {
		return isInfinity() ? this
				: getCurve().createRawPoint(getRawXCoord().multiply(scale),
						getRawYCoord(), getRawZCoords(), this.withCompression);
	}

	public ECPoint scaleY(ECFieldElement scale) {
		return isInfinity() ? this
				: getCurve().createRawPoint(getRawXCoord(),
						getRawYCoord().multiply(scale), getRawZCoords(),
	...
	public boolean equals(ECPoint other) {
		...
		if (n1 && n2) {
			...
			// TODO Consider just requiring already normalized, to avoid silent
			// performance degradation

			ECPoint[] points = new ECPoint[] { this, c1.importPoint(p2) };

			// TODO This is a little strong, really only requires coZNormalizeAll to get
			// Zs equal
		...
		return p1.getXCoord().equals(p2.getXCoord())
				&& p1.getYCoord().equals(p2.getYCoord());
	}
	...
	public static class Fp extends AbstractFp {
		...
		public ECFieldElement getZCoord(int index) {
			if (index == 1 && ECCurve.COORD_JACOBIAN_MODIFIED == this
		...
		public ECPoint add(ECPoint b) {
			...
			case ECCurve.COORD_HOMOGENEOUS: {
				...
				ECFieldElement A = u.square().multiply(w).subtract(vCubed)
						.subtract(two(vSquaredV2));

				ECFieldElement X3 = v.multiply(A);
				ECFieldElement Y3 = vSquaredV2.subtract(A).multiplyMinusProduct(u, u2,
						vCubed);
			...
			case ECCurve.COORD_JACOBIAN_MODIFIED: {
				...
				if (!Z1IsOne && Z1.equals(Z2)) {
					...
					// X3 = four(X3);
					// Y3 = eight(Y3);
					// Z3 = doubleProductFromSquares(Z1, Z2, Z1Squared,
					// Z2Squared).multiply(H);
				...
				if (coord == ECCurve.COORD_JACOBIAN_MODIFIED) {
					// TODO If the result will only be used in a subsequent addition, we
					// don't need W3
		...
		public ECPoint twice() {
			...
			case ECCurve.COORD_AFFINE: {
				...
				ECFieldElement gamma = three(X1Squared).add(this.getCurve().getA())
						.divide(two(Y1));
			...
			case ECCurve.COORD_HOMOGENEOUS: {
				...
				ECFieldElement Y3 = _4B.subtract(h).multiply(w)
						.subtract(two(_2t.square()));
			...
			case ECCurve.COORD_JACOBIAN: {
				...
				// Alternative calculation of Z3 using fast square
				// ECFieldElement Z3 = doubleProductFromSquares(Y1, Z1, Y1Squared,
				// Z1Squared);
		...
		public ECPoint twicePlus(ECPoint b) {
			...
			case ECCurve.COORD_AFFINE: {
				...
				/*
				 * Optimized calculation of 2P + Q, as described in "Trading Inversions
				 * for Multiplications in Elliptic Curve Cryptography", by Ciet, Joye,
				 * Lauter, Montgomery.
				 */
				...
				ECFieldElement L2 = two(Y1).multiply(X).multiply(dx).multiply(I)
						.subtract(L1);
		...
		public ECPoint timesPow2(int e) {
			...
			ECFieldElement X1 = this.x;
			ECFieldElement Z1 = this.zs.length < 1 ? curve.fromBigInteger(ECConstants.ONE)
					: this.zs[0];
		...
		protected ECFieldElement doubleProductFromSquares(ECFieldElement a,
				ECFieldElement b, ECFieldElement aSquared, ECFieldElement bSquared) {
			/*
			 * NOTE: If squaring in the field is faster than multiplication, then this is
			 * a quicker way to calculate 2.A.B, if A^2 and B^2 are already known.
			 */
		...
		protected ECFieldElement getJacobianModifiedW() {
			...
			if (W == null) {
				// NOTE: Rarely, twicePlus will result in the need for a lazy W1
				// calculation here
		...
		protected Fp twiceJacobianModified(boolean calculateW) {
			...
			return new Fp(this.getCurve(), X3, Y3,
					new ECFieldElement[] { Z3, W3 }, this.withCompression);
		}
	}
	...
	public static abstract class AbstractF2m extends ECPoint {
		...
		protected boolean satisfiesCurveEquation() {
			...
			if (coord == ECCurve.COORD_LAMBDA_PROJECTIVE) {
				...
				if (ZIsOne) {
					...
					// TODO If sqrt(b) is precomputed this can be simplified to a single
					// square
		...
		public ECPoint scaleX(ECFieldElement scale) {
			...
			case ECCurve.COORD_LAMBDA_AFFINE: {
				...
				ECFieldElement L = this.getRawYCoord(); // earlier JDK
				...
				return this.getCurve().createRawPoint(X, L2, this.getRawZCoords(),
						this.withCompression); // earlier JDK
			}
			case ECCurve.COORD_LAMBDA_PROJECTIVE: {
				// Y is actually Lambda (X + Y/X) here
				ECFieldElement X = this.getRawXCoord();
				ECFieldElement L = this.getRawYCoord();
				ECFieldElement Z = this.getRawZCoords()[0]; // earlier JDK
		...
		public AbstractF2m tau() {
			...
			case ECCurve.COORD_LAMBDA_AFFINE: {
				ECFieldElement Y1 = this.y;
				return (AbstractF2m) curve.createRawPoint(X1.square(),
						Y1.square(), this.withCompression);
			}
			case ECCurve.COORD_HOMOGENEOUS:
			case ECCurve.COORD_LAMBDA_PROJECTIVE: {
				ECFieldElement Y1 = this.y;
				ECFieldElement Z1 = this.zs[0];
				return (AbstractF2m) curve.createRawPoint(X1.square(),
						Y1.square(), new ECFieldElement[] { Z1.square() },
						this.withCompression);
			}
			default: {
				throw new IllegalStateException("unsupported coordinate system");
		...
		public AbstractF2m tauPow(int pow) {
			...
			case ECCurve.COORD_LAMBDA_AFFINE: {
				ECFieldElement Y1 = this.y;
				return (AbstractF2m) curve.createRawPoint(X1.squarePow(pow),
						Y1.squarePow(pow), this.withCompression);
			}
			case ECCurve.COORD_HOMOGENEOUS:
			case ECCurve.COORD_LAMBDA_PROJECTIVE: {
				ECFieldElement Y1 = this.y;
				ECFieldElement Z1 = this.zs[0];
				return (AbstractF2m) curve.createRawPoint(X1.squarePow(pow),
						Y1.squarePow(pow), new ECFieldElement[] { Z1.squarePow(pow) },
						this.withCompression);
			}
			default: {
				throw new IllegalStateException("unsupported coordinate system");
	...
	public static class F2m extends AbstractF2m {
		...
		protected ECPoint detach() {
			return new F2m(null, this.getAffineXCoord(), this.getAffineYCoord()); // earlier
																							// JDK
		}
		...
		public ECPoint add(ECPoint b) {
			...
			case ECCurve.COORD_HOMOGENEOUS: {
				...
				ECFieldElement A = uv.multiplyPlusProduct(U, VSq, curve.getA())
						.multiply(W).add(VCu);
			...
			case ECCurve.COORD_LAMBDA_PROJECTIVE: {
				...
				if (b.bitLength() < (curve.getFieldSize() >> 1)) {
					...
					if (b.isOne()) {
						...
						// TODO Can be calculated with one square if we pre-compute
						// sqrt(b)
		...
		public ECPoint twicePlus(ECPoint b) {
			...
			case ECCurve.COORD_LAMBDA_PROJECTIVE: {
				...
				ECFieldElement A = curve.getA().add(L2plus1).multiply(Z1Sq).add(L1Sq)
