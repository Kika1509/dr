...
import net.kapsch.kms.api.bouncycastle.math.ec.endo.ECEndomorphism;
import net.kapsch.kms.api.bouncycastle.math.ec.endo.GLVEndomorphism;
import net.kapsch.kms.api.bouncycastle.math.field.FiniteField;
import net.kapsch.kms.api.bouncycastle.math.field.FiniteFields;
import net.kapsch.kms.api.bouncycastle.math.raw.Nat;
import net.kapsch.kms.api.bouncycastle.util.BigIntegers;
import net.kapsch.kms.api.bouncycastle.util.Integers;
...
public abstract class ECCurve {
	...
	public static int[] getAllCoordinateSystems() {
		return new int[] { COORD_AFFINE, COORD_HOMOGENEOUS, COORD_JACOBIAN,
				COORD_JACOBIAN_CHUDNOVSKY, COORD_JACOBIAN_MODIFIED, COORD_LAMBDA_AFFINE,
				COORD_LAMBDA_PROJECTIVE, COORD_SKEWED };
	}
	...
	public ECPoint validatePoint(BigInteger x, BigInteger y) {
		...
		// note: removed because RFC 6509 parameters are causing exceptions
		// if (!p.isValid())
		// {
	...
	/**
	 * Adds <code>PreCompInfo</code> for a point on this curve, under a given name. Used
	 * by <code>ECMultiplier</code>s to save the precomputation for this
	 ...
	 * system. Where more than one point is to be normalized, this method will generally
	 * be more efficient than normalizing each point separately.
	 *
	 * @param points An array of points that will be updated in place with their
	 ...
	 * x, y coordinates reflect those of the equivalent point in an affine coordinate
	 * system. Where more than one point is to be normalized, this method will generally
	 * be more efficient than normalizing each point separately. An (optional) z-scaling
	 * factor can be applied; effectively each z coordinate is scaled by this value prior
	...
	/**
	 * Decode a point on this curve from its ASN.1 encoding. The different encodings are
	 * taken account of, including point compression for <code>F<sub>p</sub></code> (X9.62
	 * s 4.2.1 pg 17).
	...
	/**
	 * Create a cache-safe lookup table for the specified sequence of points. All the
	 * points MUST belong to this {@link ECCurve} instance, and MUST already be
	 * normalized.
	 */
	public ECLookupTable createCacheSafeLookupTable(final ECPoint[] points, int off,
			final int len) {
		...
		return new ECLookupTable() {
			...
			public ECPoint lookup(int index) {
				...
				return createRawPoint(fromBigInteger(new BigInteger(1, x)),
						fromBigInteger(new BigInteger(1, y)), false);
			}
		};
	}
	...
	public static class Fp extends AbstractFp {
		...
		public ECPoint importPoint(ECPoint p) {
			if (this != p.getCurve()
					&& this.getCoordinateSystem() == ECCurve.COORD_JACOBIAN
					&& !p.isInfinity()) {
				...
				case ECCurve.COORD_JACOBIAN_MODIFIED:
					return new ECPoint.Fp(this, fromBigInteger(p.x.toBigInteger()),
							...
							new ECFieldElement[] {
									fromBigInteger(p.zs[0].toBigInteger()) },
							p.withCompression);
				default:
					break;
				}
	...
	public static abstract class AbstractF2m extends ECCurve {
		...
		protected ECPoint decompressPoint(int yTilde, BigInteger X1) {
			...
			if (x.isZero()) {
				...
				ECFieldElement beta = x.square().invert().multiply(this.getB())
						.add(this.getA()).add(x);
		...
		/**
		 * Solves a quadratic equation <code>z<sup>2</sup> + z = beta</code>(X9.62 D.1.6)
		 * The other solution is <code>z + 1</code>.
		 *
		...
		public boolean isKoblitz() {
			return this.order != null && this.cofactor != null && this.b.isOne()
	...
	public static class F2m extends AbstractF2m {
		 ...
		 * @param a The coefficient <code>a</code> in the Weierstrass equation for
		 * non-supersingular elliptic curves over <code>F<sub>2<sup>m</sup></sub></code>.
		 * @param b The coefficient <code>b</code> in the Weierstrass equation for
		 * non-supersingular elliptic curves over <code>F<sub>2<sup>m</sup></sub></code>.
		 */
		 ...
		 */
		public F2m(int m, int k, BigInteger a, BigInteger b, BigInteger order,
				BigInteger cofactor) {
		...
		protected F2m(int m, int k1, int k2, int k3, ECFieldElement a, ECFieldElement b,
				BigInteger order, BigInteger cofactor) {
			super(m, k1, k2, k3);
		...
		protected ECCurve cloneCurve() {
			return new F2m(this.m, this.k1, this.k2, this.k3, this.a, this.b, this.order,
					this.cofactor);
		}
		...
		public ECLookupTable createCacheSafeLookupTable(ECPoint[] points, int off,
				final int len) {
			...
			return new ECLookupTable() {
				...
				public ECPoint lookup(int index) {
					...
					return createRawPoint(new ECFieldElement.F2m(m, ks, new LongArray(x)),
							new ECFieldElement.F2m(m, ks, new LongArray(y)), false);
				}
			};
		}
	}
