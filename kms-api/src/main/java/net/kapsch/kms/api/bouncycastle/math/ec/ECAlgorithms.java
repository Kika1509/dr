...
import net.kapsch.kms.api.bouncycastle.math.field.PolynomialExtensionField;
...
public class ECAlgorithms {
	...
	public static ECPoint sumOfMultiplies(ECPoint[] ps, BigInteger[] ks) {
		...
		if (endomorphism instanceof GLVEndomorphism) {
			return validatePoint(
					implSumOfMultipliesGLV(imported, ks, (GLVEndomorphism) endomorphism));
		}

		return validatePoint(implSumOfMultiplies(imported, ks));
	}

	public static ECPoint sumOfTwoMultiplies(ECPoint P, BigInteger a, ECPoint Q,
			BigInteger b) {
		...
		if (endomorphism instanceof GLVEndomorphism) {
			return validatePoint(implSumOfMultipliesGLV(new ECPoint[] { P, Q },
					new BigInteger[] { a, b }, (GLVEndomorphism) endomorphism));
		}
	 ...
	 * American Mathematical Monthly, 71(7):806-808, Aug./Sept. 1964) <pre> Input: The
	 * points P, Q, scalar k = (km?, ... , k1, k0) and scalar l = (lm?, ... , l1, l0).
	 * Output: R = k * P + l * Q. 1: Z <- P + Q 2: R <- O 3: for i from m-1 down to 0 do
	 * 4: R <- R + R {point doubling} 5: if (ki = 1) and (li = 0) then R <- R + P end if
	 * 6: if (ki = 0) and (li = 1) then R <- R + Q end if 7: if (ki = 1) and (li = 1) then
	 * R <- R + Z end if 8: end for 9: return R </pre>
	 */
	...
	/**
	 * Simple shift-and-add multiplication. Serves as reference implementation to verify
	 * (possibly faster) implementations, and for very small scalars.
	 *
	...
	static ECPoint implShamirsTrickJsf(ECPoint P, BigInteger k, ECPoint Q, BigInteger l) {
		...
		ECPoint[] table = new ECPoint[] { points[3].negate(), points[2].negate(),
				points[1].negate(), points[0].negate(), infinity, points[0], points[1],
				points[2], points[3] };
	...
	static ECPoint implShamirsTrickWNaf(ECPoint P, BigInteger k, ECPoint Q,
		...
		return implShamirsTrickWNaf(preCompP, preCompNegP, wnafP, preCompQ, preCompNegQ,
				wnafQ);
	}

	static ECPoint implShamirsTrickWNaf(ECPoint P, BigInteger k, ECPointMap pointMapQ,
			BigInteger l) {
		boolean negK = k.signum() < 0;
		boolean negL = l.signum() < 0;
		...
		int width = Math.max(2, Math.min(16,
				WNafUtil.getWindowSize(Math.max(k.bitLength(), l.bitLength()))));
	...
	private static ECPoint implShamirsTrickWNaf(ECPoint[] preCompP, ECPoint[] preCompNegP,
			byte[] wnafP, ECPoint[] preCompQ, ECPoint[] preCompNegQ, byte[] wnafQ) {
	...
	static ECPoint implSumOfMultipliesGLV(ECPoint[] ps, BigInteger[] ks,
	...
	static ECPoint implSumOfMultiplies(ECPoint[] ps, ECPointMap pointMap,
			BigInteger[] ks) {
		...
		for (int i = 0; i < halfCount; ++i) {
			...
			int width = Math.max(2, Math.min(16,
					WNafUtil.getWindowSize(Math.max(kj0.bitLength(), kj1.bitLength()))));
	...
	private static ECPoint implSumOfMultiplies(boolean[] negs, WNafPreCompInfo[] infos,
			byte[][] wnafs) {
		int len = 0;
		...
		for (int i = len - 1; i >= 0; --i) {
			...
			for (int j = 0; j < count; ++j) {
				...
				if (wi != 0) {
					...
					ECPoint[] table = (wi < 0 == negs[j]) ? info.getPreComp()
							: info.getPreCompNeg();
					r = r.add(table[n >>> 1]);
				}
			}
