 ...
 * nonadjacent form (WTNAF). The algorithms are based on the paper "Improved Algorithms
 * for Arithmetic on Anomalous Binary Curves" by Jerome A. Solinas. The paper first
 * appeared in the Proceedings of Crypto 1997.
 */
class Tnaf {
	/**
	 * The window width of WTNAF. The standard value of 4 is slightly less than optimal
	 * for running time, but keeps space requirements for precomputation low. For typical
	 * curves, a value of 5 or 6 results in a better running time. When changing this
	 * value, the <code>&alpha;<sub>u</sub></code>'s must be computed differently, see
	 * e.g. "Guide to Elliptic Curve Cryptography", Darrel Hankerson, Alfred Menezes,
	 * Scott Vanstone, Springer-Verlag New York Inc., 2004, p. 121-122
	 */
	 ...
	 */
	public static final byte[][] alpha0Tnaf = { null, { 1 }, null, { -1, 0, 1 }, null,
			{ 1, 0, 1 }, null, { -1, 0, 0, 1 } };
	/**
	 * The <code>&alpha;<sub>u</sub></code>'s for <code>a=1</code> as an array of TNAFs.
	 */
	public static final byte[][] alpha1Tnaf = { null, { 1 }, null, { -1, 0, 1 }, null,
			{ 1, 0, 1 }, null, { -1, 0, 0, -1 } };
	 ...
	 * <code><b>R</b>[&tau;]</code>.
	 * @param v The <code>&tau;</code>-adic part of the element <code>&lambda;</code> of
	 * <code><b>R</b>[&tau;]</code>.
	 * @return The norm of <code>&lambda;</code>.
	 */
	 ...
	 * element of <code><b>Z</b>[&tau;]</code>, such that their difference has minimal
	 * norm. <code>&lambda;</code> is given as
	 ...
	 * @param mu The parameter <code>&mu;</code> of the elliptic curve. Must equal 1 or
	 * -1.
	 * @return The rounded element of <code><b>Z</b>[&tau;]</code>.
	 * @throws IllegalArgumentException if <code>lambda0</code> and <code>lambda1</code>
	 * do not have same scale.
	 */
	public static ZTauElement round(SimpleBigDecimal lambda0, SimpleBigDecimal lambda1,
			byte mu) {
		int scale = lambda0.getScale();
		if (lambda1.getScale() != scale) {
			throw new IllegalArgumentException(
					"lambda0 and lambda1 do not " + "have same scale");
		}
	 ...
	 * @return The value <code>&lambda; = s k / n</code> computed to <code>c</code> bits
	 * of accuracy.
	 */
	public static SimpleBigDecimal approximateDivisionByN(BigInteger k, BigInteger s,
			BigInteger vm, byte a, int m, int c) {
		int _k = (m + 5) / 2 + c;
	...
	public static byte[] tauAdicNaf(byte mu, ZTauElement lambda) {
		...
		while (!((r0.equals(ECConstants.ZERO)) && (r1.equals(ECConstants.ZERO)))) {
			// If r0 is odd
			if (r0.testBit(0)) {
				u[i] = (byte) ECConstants.TWO
						.subtract((r0.subtract(r1.shiftLeft(1))).mod(ECConstants.FOUR))
						.intValue();
	...
	/**
	 * Applies the operation <code>&tau;()</code> to an <code>ECPoint.AbstractF2m</code>.
	 ...
	 * @param curve The elliptic curve from which to obtain <code>&mu;</code>. The curve
	 * must be a Koblitz curve, i.e. <code>a</code> equals <code>0</code> or
	 * <code>1</code> and <code>b</code> equals <code>1</code>.
	 * @return <code>&mu;</code> of the elliptic curve.
	 * @throws IllegalArgumentException if the given ECCurve is not a Koblitz curve.
	 */
	...
	/**
	 * Computes the auxiliary value <code>t<sub>w</sub></code>. If the width is 4, then
	 * for <code>mu = 1</code>, <code>t<sub>w</sub> = 6</code> and for
	 ...
	 * @param curve The elliptic curve for which to compute <code>s<sub>0</sub></code> and
	 * <code>s<sub>1</sub></code>.
	 * @throws IllegalArgumentException if <code>curve</code> is not a Koblitz curve
	 ...
	 * @param c The precision (number of bits of accuracy) of the partial modular
	 * reduction.
	...
	public static ZTauElement partModReduction(BigInteger k, int m, byte a,
		...
		// r0 = n - d0*q0 - 2*s1*q1
		BigInteger r0 = k.subtract(d0.multiply(q.u))
				.subtract(BigInteger.valueOf(2).multiply(s[1]).multiply(q.v));
	...
	/**
	 * Multiplies a {@link ECPoint.AbstractF2m ECPoint.AbstractF2m} by a
	 * <code>BigInteger</code> using the reduced <code>&tau;</code>-adic NAF (RTNAF)
	 * method.
	...
	/**
	 * Multiplies a {@link ECPoint.AbstractF2m ECPoint.AbstractF2m} by an element
	 * <code>&lambda;</code> of <code><b>Z</b>[&tau;]</code> using the
	 * <code>&tau;</code>-adic NAF (TNAF) method.
	 * @param p The ECPoint.AbstractF2m to multiply.
	 * @param lambda The element <code>&lambda;</code> of <code><b>Z</b>[&tau;]</code>.
	 * @return <code>&lambda; * p</code>
	 */
	public static ECPoint.AbstractF2m multiplyTnaf(ECPoint.AbstractF2m p,
			ZTauElement lambda) {
		ECCurve.AbstractF2m curve = (ECCurve.AbstractF2m) p.getCurve();
	 ...
	 * Multiplies a {@link ECPoint.AbstractF2m ECPoint.AbstractF2m} by an element
	 * <code>&lambda;</code> of <code><b>Z</b>[&tau;]</code> using the
	 * <code>&tau;</code>-adic NAF (TNAF) method, given the TNAF of <code>&lambda;</code>.
	 * @param p The ECPoint.AbstractF2m to multiply.
	 ...
	 * <code>&lambda;</code> of <code><b>Z</b>[&tau;]</code>.
	 * @param mu The parameter &mu; of the elliptic curve.
	 * @param lambda The element <code>&lambda;</code> of <code><b>Z</b>[&tau;]</code> of
	 * which to compute the <code>[&tau;]</code>-adic NAF.
	 ...
	 */
	public static byte[] tauAdicWNaf(byte mu, ZTauElement lambda, byte width,
			BigInteger pow2w, BigInteger tw, ZTauElement[] alpha) {
		if (!((mu == 1) || (mu == -1))) {
			throw new IllegalArgumentException("mu must be 1 or -1");
		}
